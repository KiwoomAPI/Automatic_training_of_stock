...
void CCurrentPriceDlg::InitCurPrcGrid()
{
	...
	// 열의 넓이 설정
	int i, nWidth[] = {100, 40, 100, 80, 100, 100};
...
void CCurrentPriceDlg::InitJongInfoGrid()
{
	...
	// 열의 넓이 설정
	UINT nFormat;
	int i, nWidth[] = {95, 95, 85, 105, 70, 105, 80, 105, 70, 80};
	int nCnt = sizeof(nWidth) / sizeof(*nWidth);		// 전체크기 / 원소크기 = 원소개수
	for (i = 0; i < nCnt; i++)
	{
	...
	for (i = 0; i < 5; i++)
	{
		m_grdJongInfo.SetRowHeight(i, 35);
...
void CCurrentPriceDlg::InitTickGrid()
{
	...
	// 열의 넓이 설정
	int i, nWidth[] = { 90, 100, 40, 80, 100, 100 };
...
void CCurrentPriceDlg::SetDataTickGrid(int nInsertRow, int nRow, CStringArray &arrData)
{
	...
	m_grdTick.SetRowHeight(1, 40);		// 행의 높이 설정
