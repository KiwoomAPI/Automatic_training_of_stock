...
#define DB_HOST "18.218.32.84"
#define DB_USER "root"
#define DB_PASS ""
#define DB_NAME "kiwoom"
*/
...
public:
	CString jongmokCode;
	int orderNum, orderQty, orderPrice, cheNum, chePrice, cheQty, curPrice;
};
...
void COrderDlg::OnBtnOrder()
{
	...
	//int curPrice = abs(atol(theApp.m_khOpenApi.GetCommRealData(m_strJongCode, 10)));
	...
	/*
		...
		if ((int)lPrice > curPrice) {
			lPrice = (long)curPrice;
		}
	}
	*/
	...
	//   for (i = 0; i < 10; i++)
	{
		...
		{
			//int a = _ttoi(theApp.m_khOpenApi.GetCommRealData(m_strJongCode, 10));
			int a = _ttoi(theApp.m_khOpenApi.GetCommRealData(m_strJongCode, 930));
			//int a = _ttoi(theApp.m_khOpenApi.GetCommData("OPT10001", "주식기본정보", 0, "현재가"));
			if (nCheckedOrder == IDC_ORDERTYPE_DEFAULT)
			{/// 현금(현물)주문이면...
				/*
				if (_ttoi(strPrice) > abs(_ttoi(theApp.m_khOpenApi.GetCommRealData(m_strJongCode, 10)))) {
					lRet = theApp.m_khOpenApi.SendOrder(strRQName, m_strScrNo, strAccNo, lOrderType, m_strJongCode, lQty, 
						abs(_ttoi(theApp.m_khOpenApi.GetCommRealData("182360", 10))), "00", strOrgNo);
				}
				else {
					lRet = theApp.m_khOpenApi.SendOrder(strRQName, m_strScrNo, strAccNo, lOrderType, m_strJongCode, lQty, 2000, strHogaGb, strOrgNo);
				}
				*/
			...
			/*
			if (lPrice > atol(theApp.m_khOpenApi.GetCommRealData(m_strJongCode, 10))) {
				lRet = theApp.m_khOpenApi.SendOrder(strRQName, m_strScrNo, strAccNo, 5, m_strJongCode, lQty, 
													atol(theApp.m_khOpenApi.GetCommRealData(m_strJongCode, 10)), strHogaGb, strOrgNo);
			}
			*/
...
void COrderDlg::OnReceiveTrDataKhopenapictrl(LPCTSTR sScrNo, LPCTSTR sRQName, LPCTSTR sTrcode, LPCTSTR sRecordName, LPCTSTR sPrevNext, long nDataLength, LPCTSTR sErrorCode, LPCTSTR sMessage, LPCTSTR sSplmMsg)
{
	/*
	long lRet;
	if (_ttoi(theApp.m_khOpenApi.GetCommData("OPT10012", "주문체결", 0, "주문가격")) > abs(_ttoi(theApp.m_khOpenApi.GetCommData("OPT00001", "주식기본정보", 0, "현재가")))) {
		lRet = theApp.m_khOpenApi.SendOrder(_T("주식주문"), m_strScrNo, theApp.m_khOpenApi.GetCommData("OPT10012", "주문체결", 0, "계좌번호"), 0, theApp.m_khOpenApi.GetCommData("OPT00001", "주식기본정보", 0, "종목코드"),
			_ttoi(theApp.m_khOpenApi.GetCommData("OPT10012", "주문체결", 0, "주문수량")), abs(_ttoi(theApp.m_khOpenApi.GetCommData("OPT00001", "주식기본정보", 0, "현재가"))), "00", "");
	}
	*/
...
//! Comment       : 
//******************************************************************/
OrderInfo oi[JONGNUM];
int i = 0;
CWinThread* pThread[JONGNUM] = { NULL };
...
{	
	...
	CString   strAccNo, strOrdNo, strOrdPrice, temp;
	int orderNum, orderQty, cheQty, sum;
	int orderState = 0;
	int temp2 = 0;
	long lRet;
	...
	if (theApp.m_khOpenApi.GetChejanData(905) == "+매수") {
		if (_ttoi(strGubun) == 0) {
			if (theApp.m_khOpenApi.GetChejanData(913) == "접수") {
				//AfxMessageBox("접수");

				oi[i].jongmokCode = m_strJongCode;
				oi[i].orderNum = _ttoi(theApp.m_khOpenApi.GetChejanData(9203));      //주문번호
				oi[i].orderQty = _ttoi(theApp.m_khOpenApi.GetChejanData(900));       //주문수량
				oi[i].orderPrice = _ttoi(theApp.m_khOpenApi.GetChejanData(901));     //주문가격
				oi[i].cheNum = _ttoi(theApp.m_khOpenApi.GetChejanData(909));         //체결번호
				oi[i].chePrice = _ttoi(theApp.m_khOpenApi.GetChejanData(910));       //체결가격
				oi[i].cheQty = _ttoi(theApp.m_khOpenApi.GetChejanData(911));         //체결수량
				oi[i].curPrice = _ttoi(theApp.m_khOpenApi.GetChejanData(10));        //체결가격

				temp.Format(_T("종목코드 : %d\n주문번호 : %d\n주문수량 : %d\n주문가격 : %d\n체결번호 : %d\n체결가격 : %d\n체결수량 : %d\n현재가 : %d\n%d"),
					_ttoi(oi[i].jongmokCode), oi[i].orderNum, oi[i].orderQty, oi[i].orderPrice, oi[i].cheNum, oi[i].chePrice, oi[i].cheQty, abs(oi[i].curPrice), i);
				AfxMessageBox(temp, MB_OK | MB_ICONEXCLAMATION);
				//pThread[i] = AfxBeginThread(PCheckChejan, this);
				i++;
			}
			if (theApp.m_khOpenApi.GetChejanData(913) == "체결") {					//체결(일부체결도 포함됨)
				//AfxMessageBox("체결");
				orderNum = _ttoi(theApp.m_khOpenApi.GetChejanData(9203));
				orderQty = _ttoi(theApp.m_khOpenApi.GetChejanData(900));
				cheQty = _ttoi(theApp.m_khOpenApi.GetChejanData(911));

				//체결정보 업데이트
				for (int n = 0; n < i; n++) {
					if (oi[n].orderNum == oi[i].orderNum) {
						oi[n].cheNum == _ttoi(theApp.m_khOpenApi.GetChejanData(909));
						oi[n].chePrice = _ttoi(theApp.m_khOpenApi.GetChejanData(910));
						oi[n].cheQty = _ttoi(theApp.m_khOpenApi.GetChejanData(911));
						oi[n].curPrice = _ttoi(theApp.m_khOpenApi.GetChejanData(10));
					}
				}

				temp.Format(_T("주문번호 %d : 주문 %d주, 체결 %d주"), orderNum, orderQty, cheQty);
				AfxMessageBox(temp);

				temp2 = oi[i].cheQty;

				int oPrice = _ttoi(theApp.m_khOpenApi.GetChejanData(10));
				...
				lRet = theApp.m_khOpenApi.SendOrder(_T("주식주문"), m_strScrNo, theApp.m_khOpenApi.GetChejanData(9201),
													2, m_strJongCode, atol(theApp.m_khOpenApi.GetChejanData(911)) - temp2,
													abs(oPrice), "00", "");
			}
		}
	}

	else if (theApp.m_khOpenApi.GetChejanData(905) == "-매도") {
		if (_ttoi(strGubun) == 0) {
			if (theApp.m_khOpenApi.GetChejanData(913) == "접수") {
				//AfxMessageBox("접수");
			...
			if (theApp.m_khOpenApi.GetChejanData(913) == "체결") {					//체결(일부체결도 포함)
																					//AfxMessageBox("체결");
				orderNum = _ttoi(theApp.m_khOpenApi.GetChejanData(9203));
				orderQty = _ttoi(theApp.m_khOpenApi.GetChejanData(900));
				cheQty = _ttoi(theApp.m_khOpenApi.GetChejanData(911));
	...
	MYSQL mysql;
	mysql_init(&mysql);
	if (!mysql_real_connect(&mysql, "18.218.32.84", "root", "", "kiwoom", 3306, NULL, 0))
		AfxMessageBox("error");
	...
	*/
	...
	mysql_init(&Connect);
	MYSQL *Connection = Connection = mysql_real_connect(&Connect, DB_HOST, DB_USER, DB_PASS, DB_NAME, 3306, (char*)NULL, 0);
	if (Connection == NULL)
	{
		AfxMessageBox(_T("DB 접속 실패"));
	}
	else
	{
		AfxMessageBox(_T("DB 접속 성공"));
	}
	*/
}

/*
//쓰레드 생성 시 실행 함수 (매수주문 체결 확인)
UINT PCheckChejan(LPVOID lParam) {
	CString temp;
	int orderQty, orderNum, cheQty, chePrice, curQty;
	int k = i;

	orderQty = _ttoi(theApp.m_khOpenApi.GetChejanData(900));
	orderNum = _ttoi(theApp.m_khOpenApi.GetChejanData(9203));
	curQty = _ttoi(theApp.m_khOpenApi.GetCommRealData(oi[k].jongmokCode, 930));

	while (orderQty != _ttoi(theApp.m_khOpenApi.GetCommRealData(oi[k].jongmokCode, 15))) {
		Sleep(5000);

		if (_ttoi(theApp.m_khOpenApi.GetCommRealData(oi[k].jongmokCode, 15)) > 0) {   
			cheQty = _ttoi(theApp.m_khOpenApi.GetCommRealData(oi[k].jongmokCode, 15));
			temp.Format(_T("주문번호 %d : 주문 %d주, 체결 %d주"), orderNum, orderQty, cheQty);
			AfxMessageBox(temp);
		}
	}
...
/*
//매도 모드(주문 완전 체결/부분 체결)
UINT SCheckChejan(LPVOID lParam) {
	int curPrice, orderQty, cheQty, orderNum, lPrice;
	CString temp;
	long lRet;

	orderNum = _ttoi(theApp.m_khOpenApi.GetChejanData(9203));

	COrderDlg* pView = (COrderDlg*)lParam;

	curPrice = _ttoi(theApp.m_khOpenApi.GetChejanData(10));                  //현재가
	orderQty = _ttoi(theApp.m_khOpenApi.GetChejanData(930));               //보유수량(주문수량)

	AfxMessageBox(_T("매도 모드 진입"));

	/*
	lPrice =
	lRet = theApp.m_khOpenApi.SendOrder(_T("주식주문"), pView->m_strScrNo, theApp.m_khOpenApi.GetChejanData(9201),
	2, pView->m_strJongCode, atol(theApp.m_khOpenApi.GetChejanData(911)), lPrice, "00", "");

	if (!theApp.IsError(lRet))
	{
	while (orderQty != _ttoi(theApp.m_khOpenApi.GetChejanData(911))) {
	Sleep(5000);

	if (orderQty == _ttoi(theApp.m_khOpenApi.GetChejanData(911))) {         //주문수량과 체결수량이 같으면
	cheQty = _ttoi(theApp.m_khOpenApi.GetChejanData(911));
	temp.Format(_T("주문번호 %d : 체결 완료"), orderNum);
	AfxMessageBox(temp);
	}

	else if (_ttoi(theApp.m_khOpenApi.GetChejanData(911)) > 0) {         //체결수량이 0보다 크면
	cheQty = _ttoi(theApp.m_khOpenApi.GetChejanData(911));
	temp.Format(_T("주문번호 %d : 주문 %d주, 체결 %d주"), orderNum, orderQty, cheQty);
	AfxMessageBox(temp);
	}
